...
FHIR also specifies a `mustSupport` flag for elements; when this is active, it signals that support is expected from users. Exactly what the expectation is must be specified further. In this guide, `mustSupport` means that the further specification follows from the obligations.
